...
7. 玩家按 `Tab` 打开装备圆环，用数字键从已获得的装备/室内物品里更换当前装备；后续树木/采集物交互可读取装备状态。
