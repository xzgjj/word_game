...
- `Q` 作为交互面板的统一返回/关闭键；`Esc` 只负责全局系统菜单，避免木牌、游戏机和未来交互系统互相抢键。
...
- 树枝、石子、花种、河贝和鱼影在采集/钓到后会从场景里消失；按资源类型固定刷新时间，后续新增同类资源沿用同一时间表。
...
| `F5` | 手动记录档案 |
| `S` / `L` / `Delete` | 系统存档菜单中保存手动档、读取、删除手动档 |
| `Enter` | 游戏机菜单打开后进入小游戏 |
