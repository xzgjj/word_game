...
- 项目本质：当前是《星绪森林》0-1 阶段的小游戏设计与概念交互演示，不是完整游戏工程。位置：`README.md`、本地 `doc/design_document.md`。
...
项目对标和参考：主要吸收小岛生活、木屋和资源采集、定点微建造、现代入口风格和像素模式切换。详细权重与评分保存在本地 `doc/visual_alignment_standard.md`，README 不再展示对标内容。
